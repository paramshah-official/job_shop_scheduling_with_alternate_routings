...
def whale_swarm_algorithm(job_info, num_machines):
    ...
    for generation in range(termination_criterion):
        ...
        # Crossover
        for ind in range(2, len(population)):
            if random.random() < crossover_rate:
                x = 0
                if random.random() > 0.6:
                    x = 1
                parent1, parent2 = population[x], population[ind]
                offspring1, offspring2 = crossover(parent1, parent2, job_info)
                population += [offspring1, offspring2]

        # Mutation
        if generation < 0.2*termination_criterion:
            mutation_rate = 0.09
        elif generation < 0.5*termination_criterion:
            mutation_rate = 0.05
        else:
            mutation_rate = 0.01
...
best_solution, best_fitness = whale_swarm_algorithm(jobs_operations_dict, num_machines)
